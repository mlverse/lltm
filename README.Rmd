...
- **src/code.cpp**: In this file we implement the wrappers that takes R obejcts
  ...
  In the above code chunk the `XPtrTorchTensor` type allow us to take torch tensors
  directly from R. `XPtrTorchTensorList` will convert a `std::vector<torch::Tensor>`
